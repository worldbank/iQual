...
module_dict_list  = [{f:module for f in funcmethods} for module, funcmethods in module_options.items()]
...
def load_estimator(model,sklearn_estimator_options=estimator_options):
    """ 
    """
...
class ThresholdPredictor(BaseEstimator):
    ...
    def __init__(
        ...
        self.thresholds      = np.linspace(self.threshold_range[0], self.threshold_range[1], self.steps)
    ...
    def fit(self, X=None, Y=None, *args, **kwargs):
        ...
        if self.use_threshold is True:
            ...
            scores = [self.scorer.calc_scores(Y, y_preds)[self.scoring_metric].values[0] for y_preds in y_pred_array]
...
class Estimator(BaseEstimator, TransformerMixin):
    ...
    def __init__(self,model,is_final=False,**kwargs) -> callable:
        ...
        self.estimator      = load_estimator(model=self.model)(**kwargs)

    def get_params(self, **kwargs) -> dict:
        params = self.estimator.get_params(**kwargs)
    ...
    def set_params(self, **kwargs):        
        ...
        self.estimator = load_estimator(model=str(self.model))()
        self.estimator.set_params(**{k:v for k,v in kwargs.items() if k not in ['model','is_final']})
    ...
    def get_normalized_decisions(self, X, **kwargs):
        ...
        if hasattr(self.estimator, 'decision_function'):
            decisions = self.estimator.decision_function(X, **kwargs)
        elif hasattr(self.estimator, '_decision_function'):
            decisions = self.estimator._decision_function(X, **kwargs)
        else:
    ...
    def predict_proba(self, X, **kwargs):
        ...
        if hasattr(self.estimator, "predict_proba"):
            ...
            if hasattr(self.estimator, 'decision_function') or hasattr(self.estimator, '_decision_function'):                          
