# /usr/bin/env python
"""
    This module contains `CrossValidation` class,
    also inherited by the `NLP` model during training.
"""
...
class CrossValidator:
    ...
    def fit(self, X, Y, **kwargs):
        ...
        self.cv_scores = self.param_df.loc[
            self.param_df[self.rank_column ] == 1, [f"split{i}_test_{self.refit_string}" for i in range(self.n_splits)]].to_dict("records")[0]
    ...
    def get_cv_results(self):
        ...
        self.param_df = self.param_df.sort_values(by=self.rank_column,ignore_index=True)
