...
from estimation import Estimator, ThresholdPredictor
...
def construct_text_pipelines(
    ...
    **kwargs
    ...
    if q_col is not None:
        q_pipe = Pipeline(
                [
                    ("selector",FunctionTransformer(column_selector,kw_args=dict(column_name=q_col),validate=False,),),
                    ("vectorizer",Vectorizer(model,env=env,**kwargs),)
                ]
        )
    if a_col is not None:
        a_pipe = Pipeline(
                [
                    ("selector",FunctionTransformer(column_selector,kw_args=dict(column_name=a_col),validate=False,),),
                    ("vectorizer",Vectorizer(model,env=env,**kwargs),)
                ]
        )
...
class NLPModel:
    ...
    def add_feature_scaling(self,scaler_name="StandardScaler", **kwargs):
        """
        Scale the features
        Args:
            scaler_name (str):
                Name of the scaler method
            **kwargs:
                Keyword arguments for the scaler method
        """
        self.transformation_steps.append(("Scaling", FeatureScaler(scaler_name=scaler_name, **kwargs)))
    
    def add_feature_reduction(self, reducer_name="PCA", **kwargs):
        """
        Reduce the features
        Args:
            reducer_name (str): Name of the reducer method
            **kwargs: Keyword arguments for the reducer method
        """
        self.transformation_steps.append(("DimensionalityReduction", DimensionalityReduction(reducer_name=reducer_name, **kwargs)))

    def add_estimation(self, name="LogisticRegression", **kwargs):
        """
        """
        self.estimators         = name
        self.kwargs   = kwargs
        self.estimation_steps.append(("Estimation",Estimator(name, **kwargs)))

    def add_threshold(self, **kwargs):
        """
        Add a custom threshold to the pipeline
        """
        self.post_estimation_steps.append(("Threshold", ThresholdPredictor(**kwargs)))
    ...
    def cross_validate_fit(
        ...
        **kwargs,
        ...
        """
        print(f".......{count_hyperparameters(search_parameters)} hyperparameters configurations possible.....\r",end="")
    ...
    def predict_proba(self, X):
        ...
        if hasattr(self.model, "predict_proba"):
            ...
            if probas.ndim == 2:
                ...
                raise ValueError(
                    "The predicted probabilities have an unexpected shape"
                )
        else:
